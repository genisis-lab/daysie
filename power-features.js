...
(() => {
  ...
  async function loadFamilyDashboard() {
    ...
    try {
      ...
      target.innerHTML = `
        <div class="family-summary"><h4>Who’s available</h4>${familyDashboard.members.map((member) => `<p><span class="availability-dot ${safe(member.availability)}"></span>${safe(member.emoji)} ${safe(member.name)} · ${safe(member.availability)}</p>`).join("")}</div>
