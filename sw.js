...
;(() => {
  ...
  const DONE_KEY='daysie.familyInbox.done.v3';
  ...
  loadFamilyInbox=async function(){ if(!settings.authToken) return; try{ const res=await fetch(API+'/family/inbox?ts='+Date.now(),{headers:authHeaders(),cache:'no-store'}); if(!res.ok) return; const d=await res.json(); const done=doneIds(); window.familyInbox=(d.items||[]).filter(it=>!done.has(it.id)); renderFamilyInbox(); }catch(e){} };
  ...
  if('serviceWorker' in navigator&&!window.__daysieLiveListFix){ window.__daysieLiveListFix=1; navigator.serviceWorker.addEventListener('message',ev=>{ if(ev.data&&ev.data.type==='family-list-updated'){ window.familyListsLocalEditAt=0; try{loadFamilyLists();}catch(e){} try{loadFamilyInbox();}catch(e){} } }); }
  setInterval(()=>{ if(document.visibilityState==='visible'){ try{loadFamilyLists();loadFamilyInbox();}catch(e){} } },5000);
...
self.addEventListener('fetch',event=>{
  ...
  if(url.origin===self.location.origin){
    ...
    if(p.endsWith('/app3.js')){event.respondWith(fetch(req,{cache:'no-store'}).then(r=>r.text()).then(t=>js(t.includes('__daysieLiveListFix')?t:t+'\n'+APP3_FIX)).catch(()=>caches.match(req).then(c=>c||caches.match('./index.html'))));return;}
    ...
    if(p.endsWith('/index.html')||p==='/'||p.endsWith('/')){event.respondWith(fetch(req,{cache:'no-store'}).then(r=>r.text()).then(t=>txt(t.replace(/\s*<div id="familyListsSection" class="hidden">[\s\S]*?<div id="familyListsList" class="lists-list"><\/div>\s*<\/div>/,''),'text/html; charset=utf-8')).catch(()=>caches.match(req).then(c=>c||caches.match('./index.html'))));return;}
