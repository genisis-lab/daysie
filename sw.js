...
const CACHE_NAME = 'daysie-v16-hotfix';
...
;(() => {
  ...
  if('serviceWorker' in navigator&&!window.__daysieLeaveLiveFixV16){ window.__daysieLeaveLiveFixV16=1; navigator.serviceWorker.addEventListener('message',ev=>{ if(ev.data&&ev.data.type==='family-list-updated'){ window.familyListsLocalEditAt=0; try{loadFamily();loadFamilyLists();loadFamilyInbox();}catch(e){} } }); }
...
self.addEventListener('fetch',event=>{
  ...
  if(url.origin===self.location.origin){
    ...
    if(p.endsWith('/app.js')){event.respondWith(fetch(req,{cache:'no-store'}).then(r=>r.text()).then(t=>{t=t.replace(/const APP_VERSION = ['\"][^'\"]+['\"];/,"const APP_VERSION = '2026.06.08-16';");return js(t.includes('fixedGo(tab)')?t:t+'\n'+APP1_FIX)} ).catch(()=>caches.match(req).then(c=>c||caches.match('./index.html'))));return;}
    ...
    if(p.endsWith('/app3.js')){event.respondWith(fetch(req,{cache:'no-store'}).then(r=>r.text()).then(t=>js(t.includes('__daysieLeaveLiveFixV16')?t:t+'\n'+APP3_FIX)).catch(()=>caches.match(req).then(c=>c||caches.match('./index.html'))));return;}
